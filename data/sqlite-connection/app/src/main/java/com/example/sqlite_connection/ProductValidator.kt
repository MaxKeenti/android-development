...
data class ProductFormErrors(
    ...
    val hasErrors get() = name != null || price != null || country != null || stock != null
}

fun validateProductForm(name: String, price: String, country: String, stock: String): ProductFormErrors {
    val nameError = when {
    ...
    val countryError = when {
        country.isBlank() -> "El país no puede estar vacío"
        else -> null
    }
    val stockError = when {
    ...
    return ProductFormErrors(nameError, priceError, countryError, stockError)
}
